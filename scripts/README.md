...
r/bangalore, uses Claude (`claude-opus-4-8`) to extract specific named
places/events, geocodes them inside Bengaluru (Nominatim), and records rows in
the `mentions` table. Those feed `trending_score` via the time-decayed view in
`supabase/migrations/0001_init.sql`, so mentioned spots climb the ranking on
their own.
...
3. Claude extracts specific, visitable places/events — skipping civic rants,
...
Without the two Supabase vars **or** the Anthropic key, the script auto-forces
