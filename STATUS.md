...
**Last updated:** 2026-07-29 · **Phase:** Live on Netlify (prod). Newsletter now curates 10 diverse picks (round-robin across Eat/Drink/Do/See, area-deduped); the weekly send is still blocked on a verified Resend domain + `NEWSLETTER_FROM` — see Pending activation. Previously: UX/scale batch **committed + pushed to `main`** (commit `5c14cb1`) — clustering, share links, on-site newsletter, coffee button, admin full-info + merge, tighter dedup. Netlify deploy triggered (1 of ~2 remaining prod runs used). The one live duplicate found (Cubbon Park, curated vs. reddit row ~230m apart) was merged by hand directly in Supabase — reddit mention moved over, dup row deleted, trending score preserved. Pending: run migration `0011_merge_places.sql` so the admin Merge button works for future dupes; optionally set `NEXT_PUBLIC_BMC_URL`.
